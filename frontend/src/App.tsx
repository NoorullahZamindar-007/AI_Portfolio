...
function ProfileImage({ className = "" }: { className?: string }) {
  ...
  if (hasError) {
    return (
      <img 
        src="https://ui-avatars.com/api/?name=Noorullah+Zamindar&size=256&background=f1f5f9&color=334155" 
        alt="Noorullah Zamindar" 
        className={className} 
      />
    );
  }
...
function getLocalPortfolioReply(query: string): { intent: ChatMessage['type']; ai_text: string } {
  ...
  if (q.includes('contact') || q.includes('email') || q.includes('linkedin') || q.includes('github')) {
    return {
      intent: 'contact',
      ai_text: "You can reach Noorullah through the contact info above. The best direct email is noorullahzamindar9@gmail.com."
    };
  }

  if (q.includes('education') || q.includes('university') || q.includes('study') || q.includes('studied') || q.includes('learn') || q.includes('college') || q.includes('school') || q.includes('degree') || q.includes('bachelor') || q.includes('rana') || q.includes('alagappa') || q.includes('storai')) {
    return {
      intent: 'general',
      ai_text: "Noorullah studied Bachelor of Computer Science in Software Engineering at Rana University, Afghanistan. He is also pursuing a Bachelor of Business Administration at Alagappa University, India through an ICCR scholarship. He completed a Diploma in English Language at Storai Institute, Afghanistan, and high school at Abdul Qayum Wardak High School, Afghanistan."
    };
  }

  if (q.includes('project') || q.includes('portfolio') || q.includes('emotion') || q.includes('email agent') || q.includes('bot') || q.includes('invoice') || q.includes('reservation') || q.includes('pos')) {
    return {
      intent: 'projects',
      ai_text: "I have loaded Noorullah's project portfolio above, including AI automation, machine learning, chatbot, OCR, web app, and POS analytics work."
    };
  }

  if (q.includes('skill') || q.includes('stack') || q.includes('technology') || q.includes('python') || q.includes('machine learning') || q.includes('automation')) {
    return {
      intent: 'skills',
      ai_text: "Noorullah's core strengths are Python, data science, machine learning, AI automation, Flask, SQL, dashboards, and workflow tools like Zapier, Make, and n8n."
    };
  }

  if (q.includes('experience') || q.includes('resume') || q.includes('work') || q.includes('job') || q.includes('trainer') || q.includes('undp')) {
    return {
      intent: 'resume',
      ai_text: "I have displayed Noorullah's professional timeline above, covering AI training, data science, system management, teaching, freelancing, and technical support roles."
    };
  }

  if (q.includes('about') || q.includes('yourself') || q.includes('who are you') || q.includes('noorullah')) {
    return {
      intent: 'me',
      ai_text: "You can see a quick summary of Noorullah's background above. Ask about his AI projects, teaching experience, automation work, or data science stack."
    };
  }

  return {
    intent: 'general',
    ai_text: "I can help you explore Noorullah Zamindar's AI/data science profile, projects, skills, experience, education, and contact details."
  };
}
        ...
        <div className="flex-1">
          <AnimatePresence>
            {viewState === 'landing' && (
              <motion.a 
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                href="https://noorullahzamindar-007.github.io/Portfolio/" 
                target="_blank" 
...
function LandingView({ onQuery, theme }: { onQuery: (q: string, t: ChatMessage['type']) => void, theme?: string }) {
  ...
  const dynamicPlaceholder = useTypewriterPlaceholder([
    "Ask me anything...",
    "Ask me about Facial Emotion Recognition...",
    "What is Noorullah's AI stack?",
    "Tell me about your UNDP training experience...",
    ...
    <>
      ...
      >
        <div className="text-center mb-6">
          <h2 className={`text-2xl sm:text-3xl font-serif italic mb-3 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-300' : 'text-slate-700'}`}>Hey, I'm Noorullah Zamindar</h2>
  ...
  return (
    <div className="w-full flex flex-col md:flex-row items-center md:items-start gap-8 mb-8">
      ...
      <div className="flex-1 flex flex-col text-left">
        <h1 className={`text-4xl font-bold mb-2 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-100' : 'text-slate-900'}`}>Noorullah Zamindar</h1>
        <p className={`mb-4 font-medium transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-400' : 'text-slate-505'}`}>AI Engineer & Data Scientist <span className="mx-2">-</span> Kabul, Afghanistan</p>
        
        <p className={`leading-relaxed mb-6 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-300' : 'text-slate-700'}`}>
          I am an AI Engineer and Data Scientist with experience in machine learning, data analytics, Python development, AI automation, and intelligent system development. I build practical AI solutions such as predictive models, AI agents, automation workflows, dashboards, NLP tools, and web-based AI applications.
        </p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">Python</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">AI Automation</span>
        </div>
      </div>
    </div>
  );
...
function ContactCard({ theme }: { theme?: string }) {
  const socials = [
    { name: 'LinkedIn', url: 'https://www.linkedin.com/in/noorullah-zamindar-4975a328a/' },
    { name: 'GitHub', url: 'https://github.com/NoorullahZamindar-007' },
    { name: 'Portfolio', url: 'https://noorullahzamindar-007.github.io/Portfolio/' }
  ];
